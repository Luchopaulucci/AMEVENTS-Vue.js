<template>
    <div>
        ...
        {{ superFiltro }}
        <section class="cards">
            <p v-if="!eventosUpcoming.length" class="error">Not found</p>
            <div v-for="evento of eventosUpcoming" class="card" :key="evento._id">
...
export default {
    data() {
        return {
            ...
            eventosUpcoming: [], //TIENE LOS EVENTOS UPCOMING
    ...
    created() {
        ...
        console.log("ESTOY EN EL UPCOMING");
    },
    mounted() {
        ...
        pedirDatos() {
            fetch(this.apiUrl)
                ...
                .then(datosApi => {
                    ...
                    this.eventosBackup = this.eventosUpcoming    //GUARDA SOLO LOS EVENTOS EN EL ARRAY EVENTOSBACKUP
                    this.sacarCategorias(this.eventosUpcoming);
                })
        },
        ...
        filtrarFechas(array) {
            for (let i = 0; i < array.length; i++) {
                if (array[i].date > this.date) {
                    this.eventosUpcoming.push(array[i]);
                }
            }
        },
    ...
    computed: {  //ya escucha el cambio en la propiedad no hace falta llamar al evento
        superFiltro() {
            ...
            if (!this.categoryCheckbox.length) {
                ...
                this.eventosUpcoming = primerFiltro.filter(evento =>
